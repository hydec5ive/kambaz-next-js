  ...
  return (
    <div className="p-4" id="wd-dashboard">
      <h1 id="wd-dashboard-title">
        Dashboard
        <Button variant="primary" className="float-end" onClick={() => setShowAllCourses(!showAllCourses)}>
          Enrollments
        </Button>
      </h1>
      <hr />
        ...
        {filteredCourses.map((crs: any) => (
          <Col key={crs._id} className="wd-dashboard-course" style={{ width: "300px" }}>
            <Card>
              ...
              <CardBody>
                {isEnrolled(crs._id) && (
                  <Link href={`/courses/${crs._id}/home`} className="btn btn-primary btn-sm me-2">Go</Link>
                )}
                {isFaculty && (
                  <>
                    ...
                    <Button variant="danger" size="sm" className="me-2" id="wd-delete-course-click"
                      onClick={(event) => {
                        event.preventDefault();
